...
class PXB1Hits : public edm::one::EDAnalyzer<edm::one::SharedResources> {
    public:
        explicit PXB1Hits(const edm::ParameterSet&);
        ~PXB1Hits();
    ...
    private:
        ...
        TTree *tree_;
        int run_, lumi_, bx_, instLumi_, npv_, good_vertices_; uint64_t event_; 
        ...
        float hit_global_phi_, hit_global_z_, hit_local_x_, hit_local_y_, hit_sizeX_, hit_sizeY_, hit_firstpixel_x_, hit_firstpixel_y_, hit_lastpixel_x_, hit_lastpixel_y_, hit_chi2_, hit_charge_, hitInRandomWindowDistance_;
        int hit_edg_, hit_cent_, ngoodvalid_, nbadvalid_, diffpixelx_, diffpixely_;
        double nextvhcharge_, chargevalid_;

        int diffmisspixex_, diffmisspixely_;
        double nextmischarge_;
...
//
PXB1Hits::PXB1Hits(const edm::ParameterSet& iConfig):
    ...
    //pixelQualityLabel_(iConfig.getParameter<std::string>("SiPixelQuality"))
{
    ...
    tree_->Branch("hit_lastpixel_x", &hit_lastpixel_x_, "hit_lastpixel_x/F");
    tree_->Branch("hit_lastpixel_y", &hit_lastpixel_y_, "hit_lastpixel_y/F");
    ...
    tree_->Branch("ngoodvalid", &ngoodvalid_, "ngoodvalid/i");
    tree_->Branch("nbadvalid",  &nbadvalid_,  "nbadvalid/i");
    
    tree_->Branch("hit_cent", &hit_cent_, "hit_cent/i");
    tree_->Branch("hit_edg",  &hit_edg_,  "hit_edg/i");
    
    tree_->Branch("diffpixelx", &diffpixelx_, "diffpixelx/i");
    tree_->Branch("diffpixely", &diffpixely_, "diffpixely/i");
    tree_->Branch("nextvhcharge", &nextvhcharge_, "nextvhcharge/d");
...
PXB1Hits::~PXB1Hits()
{
    ...
    void PXB1Hits::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    using namespace edm;
    run_  = iEvent.id().run();
    ...
    for (const reco::Candidate & pair : *pairs) {
        ...
        for (const auto & detAndState : compDets) {
            ...
            hitInRandomWindow_ = false; hitInRandomWindowDistance_ = 0.1;
            for (const auto & hit : allhits) { 
            ...
            for (const auto & hitAndChi2 : rechitsAndChi2) {
                ...
                hit_lastpixel_x_  = clustref->maxPixelRow();
                hit_lastpixel_y_  = clustref->maxPixelCol();
                hit_chi2_ = hitAndChi2.first;
                hit_charge_ = clustref->charge()*chargeCorr;
                ...
                //Module config
                DetId wherePXB1 = pixhit->geographicalId();
                auto temp = trackergeom.idToDet(wherePXB1);
                ...
                //std::cout << " x_first = " << hit_firstpixel_x_ << " | x_last = " << hit_lastpixel_x_ << std::endl;
                //std::cout << " y_first = " << hit_firstpixel_y_ << " | y_last = " << hit_lastpixel_y_ << std::endl;
                //std::cout << " charge = " << hit_charge_ << " | Valid? = " << hitOnTrack_ << std::endl;
                // Ratio bad/good charge
                if ((trackHasHit_ = 1) && (hit_charge_ < 20000))  ++nbadvalid_;
                else if ((trackHasHit_ = 1) && (hit_charge_ >= 20000)) ++ngoodvalid_;
                // Efficiency in function of detector geometry (Split hits in central or edge Region)
                if ( ( hit_firstpixel_x_ < (0.15*nRowSiPixel) || hit_lastpixel_x_ > (nRowSiPixel - (0.15*nRowSiPixel)) ) 
                  || (hit_firstpixel_y_ < (0.15*nColSiPixel) || hit_lastpixel_y_ > (nColSiPixel - (0.15*nColSiPixel))) ){
                  hit_edg_++;
                ...
                // Extra hit
                auto cl_iter = pixelC->find(wherePXB1);
                if (cl_iter == pixelC->end()) {
                  //std::cout << "  ... no pixel clusters on this detid" << std::endl;
                } else {
                  edmNew::DetSet<SiPixelCluster> clustersextra = *cl_iter;
                  int diffpixelx = 10;
                  int diffpixely = 10; 
                  double nextvhcharge = -999.;
                  for (const SiPixelCluster &clusterextra : clustersextra) {
                    //std::cout << "  Cluster of " << clusterextra.size() << " pixel: " << std::endl;
                    float charge = clusterextra.charge();                   
                    for (unsigned int sx = clusterextra.minPixelCol(), mx = clusterextra.maxPixelCol()+1; sx < mx; ++sx) {
                      for (unsigned int sy = clusterextra.minPixelRow(), my = clusterextra.maxPixelRow()+1; sy < my; ++sy) {
                        if ( ( charge != hit_charge_ ) && (sx < hit_firstpixel_x_) && (std::abs(sx-hit_firstpixel_x_) <= diffpixelx) 
                         && ( ( (sy < hit_firstpixel_y_) && (std::abs(sy-hit_firstpixel_y_) <= diffpixely) ) 
                         || ( (sy > hit_lastpixel_y_) && (std::abs(sy-hit_lastpixel_y_) <= diffpixely) ) ) ){
                          diffpixelx = std::abs(sx - hit_firstpixel_x_);
                          if (sy < hit_firstpixel_y_) diffpixely = std::abs(sy-hit_firstpixel_y_);
                          else diffpixely = std::abs(sy-hit_lastpixel_y_);
                          nextvhcharge = charge;
                        } else if ( ( charge != hit_charge_ ) && (sx > hit_lastpixel_x_) && (std::abs(sx-hit_lastpixel_x_) <= diffpixelx) 
                         && ( ( (sy < hit_firstpixel_y_) && (std::abs(sy-hit_firstpixel_y_) <= diffpixely) ) 
                         || ( (sy > hit_lastpixel_y_) && (std::abs(sy-hit_lastpixel_y_) <= diffpixely) ) ) ){
                          diffpixelx = std::abs(sx-hit_lastpixel_x_);
                          if (sy < hit_firstpixel_y_) diffpixely = std::abs(sy-hit_firstpixel_y_);
                          else diffpixely = std::abs(sy-hit_lastpixel_y_);
                          nextvhcharge = charge;
                        }
                        //std::cout << "Extra hit, min x = " << hit_firstpixel_x_-sx << " | max x = " << sx-hit_lastpixel_x_ << " | min y = " << hit_firstpixel_y_-sy << " | max y = " << sy-hit_lastpixel_y_ << " | charge = " << charge << std::endl;
                      }
                    }
                  }
                  if ( diffpixelx < 10 && diffpixely < 10 && nextvhcharge != hit_charge_ ) {
                    diffpixelx_ = diffpixelx;
                    diffpixely_ = diffpixely;
                    nextvhcharge_ = nextvhcharge;
                    //std::cout << " EXTRA HIT, diffpixelx = " << diffpixelx_ << " | diffpixely = " << diffpixelx_ << " | charge = " << nextvhcharge_ << std::endl;
                  }
                }        
                //Save in Tree
                ...
                if (debug_) printf("             rechit x = %+6.3f +- %5.3f   y = %+6.3f +- %5.3f     dx = %+6.3f +- %5.3f   dy = %+6.3f +- %5.3f   chi2 = %7.2f    ontrack = %c  ",  
                                    ...
                                    (hitOnTrack_ ? 'Y' : 'n'));
                //if (debug_) printf(" size: %3d (%2d x %2d), raw charge %8f   corr charge %8.1f/n", clustref->size(), clustref->sizeX(), clustref->sizeY(), clustref->charge(), (clustref->charge()*chargeCorr));
                if (debug_) {
